...
for var_for_scan=[1 1.6];
    ...
    el.sigma_e_x=5*1e-6; % IP vertical electron beam size [m]
    el.sigma_e_y=5*1e-6; % IP horizontal electron beam size [m]
    el.norm_emit_x=2*1e-4; %Normilized emittance x [m rad]
    el.norm_emit_y=2*1e-4; %Normilized emittance y [m rad]
    ...
    name=['cain_out_' num2str(el.initial_beam_energy_MeV) '_GeV_ES_' num2str(el.energy_spread_initial) '_norm_em_' num2str(el.norm_emit_x) ];
