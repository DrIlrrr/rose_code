%  make_path
function data_for_formula(BASE_DIRECTORY,name)
...
photons_data=load([ BASE_DIRECTORY 'photon_data_plots/photons_data.dat']);%photons_data=load([out_folder 'photon_data_plots/photons_data.dat']);
beam_property=load([ BASE_DIRECTORY 'cain_tmp/exp.dat']);%beam_property=load([out_folder 'cain_tmp/exp.dat']);
...
gamma_f=mean(e_E)/(0.511e6);
delta_gamma=std(e_E./(0.511e6));
norm_em_x=sqrt(gamma_f^2-1)*e_emx_sqrt;
norm_em_y=sqrt(gamma_f^2-1)*e_emy_sqrt;
...
a_0=4.3*(laserwl/W0)*sqrt(rflags.pulseE/rflags.sigt);
 ...
 save([name '.dat'],'theta_max','delta_gamma','gamma_f','norm_em_x','sigma_0',...
     'norm_em_y','sigma_0y','laser_bandwidth','M2','laserwl','W0','a_0',...
     'theta_max','bandwith_cm','e_emx_sqrt','e_emy_sqrt','e_E','full_spectrum')
