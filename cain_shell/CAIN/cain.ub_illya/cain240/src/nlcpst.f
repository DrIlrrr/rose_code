...
!     %     XIMAX,LMMAX0,LWGTCP,WGTCPF,IRTN)
 ...
 900  IRTN=1000
      WRITE(MSGFL,905)
 905  FORMAT(' (SUBR.NLCPST) Memory insufficient.')
      GOTO 990
 910  IRTN=1001
      WRITE(MSGFL,915) MMY
 915  FORMAT(' (SUBR.NLCPST) MY too large. Must be <=',I4)
      GOTO 990
 920  IRTN=1002
      WRITE(MSGFL,925) MMPH
 925  FORMAT(' (SUBR.NLCPST) MPH too large. Must be <=',I4)
      GOTO 990
 930  IRTN=1003
      WRITE(MSGFL,935) MMXI
 935  FORMAT(' (SUBR.NLCPST) MXI too large. Must be <=',I4)
      GOTO 990
 940  IRTN=1004
      WRITE(MSGFL,945) MMLM
 945  FORMAT(' (SUBR.NLCPST) MLAM too large. Must be <=',I4)
      GOTO 990
 950  IRTN=1100
