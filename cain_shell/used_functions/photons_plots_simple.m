function photons_plots_simple(out_folder)
global rflags
...
phot_angle=atan(sqrt(photons_data(:,9).^2+photons_data(:,10).^2)./photons_data(:,11));
...
figure(ifig)
ifig=ifig+1;
histogram2(phot_angle(faa),full_spectrum(faa),[100 100],'DisplayStyle','tile','ShowEmptyBins','on')
% grid on
set(gca,'FontSize',16)
ylabel('energy of scattered photons')
xlabel('scattered angle')
...
figure(ifig)
ifig=ifig+1;
histogram2(phot_angle(faa),full_spectrum(faa),[100 100],'FaceColor','flat')
% grid on
set(gca,'FontSize',16)
ylabel('energy of scattered photons')
xlabel('scattered angle')
...
figure(ifig)
ifig=ifig+1;
bar(linspace(0,max(full_spectrum/1e3),nbin_plot),smooth(hist(full_spectrum/1e3,nbin_plot)*weigth))
set(gca,'FontSize',16)
...
ifig=ifig+1;
hold on
bar(linspace(0,max(full_spectrum),nbin_plot),smooth(hist(full_spectrum,nbin_plot)*weigth))
hold off
grid on
ylim([0 max(smooth(hist(full_spectrum,nbin_plot)*weigth))])
set(gca,'FontSize',16)
ylabel('number of scattered photons')
xlabel('photons energy (MeV)')
...
figure(ifig)
ifig=ifig+1;
histogram2(full_spectrum,phot_angle,[50 50],'DisplayStyle','tile','ShowEmptyBins','on');
set(gca,'FontSize',16)
...
ind_23=find(phot_angle<0.005);
figure(ifig)
ifig=ifig+1;
histogram2(full_spectrum(ind_23),phot_angle(ind_23),[50 50],'DisplayStyle','tile','ShowEmptyBins','on');
set(gca,'FontSize',16)
xlabel('Energy','FontSize',20)
ylabel('\theta','FontSize',20)
zlabel('# \gamma','FontSize',20)
...
for ni=diapason;
 ...
 if bandwith_cm(qq)>=0.048 && bandwith_cm(qq)<=0.51 % find angle for given bandwith
 el_angel=el_angel_1;
% stop;
 end
end
