function plot_expdat(out_folder)

beam_in=load([out_folder 'cain_tmp/exp.dat']);%beam_property=load([out_folder 'cain_tmp/exp.dat']);
...
filename = [out_folder 'initial_beam_plot/EXP_fig_' num2str(ifig)];
fname = [ filename '.png'];
print('-dpng', fname);
...
save([out_folder 'initial_beam_plot/beam_mat.dat'],'e_emx_sqrt','e_emy_sqrt','sigma_0x','sigma_0y',...
...
f_name=[out_folder 'initial_beam_plot/beam_stat.txt'];
...
f_name=[out_folder 'initial_beam_plot/beam_stat_TEX.txt'];
%  filename = sprintf('NEW_plot.txt',tilted_name);
fileID = fopen(f_name,'w');
fprintf(fileID,'Electron beam non norm Emittances:\\\\ \n');
fprintf(fileID,'Emit X $= %10.5e $\\\\ \n',e_emx_sqrt);
