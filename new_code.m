...
for scan_var=[0.99 0.999];
    L=5e-3;
    ifig=1;
    gvar.lim_for_cos_theta_cm=scan_var;
    gvar.energy=260;
    ...
    save_dir_start=[pwd '/Moller_cos_lim_' num2str(gvar.lim_for_cos_theta_cm) '_ib_51_' num2str(gvar.energy) '_sigma_end_cut_' num2str(gvar.sigma_cut_at_end) '_L_' num2str(L) '/'];mkdir(save_dir_start);
    %% Load or create beams
    if rflags.use_ideal_beams==0

        rflags.moller=1;% M??ller scattering
